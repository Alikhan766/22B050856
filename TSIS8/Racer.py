...
background = pygame.image.load("racer/background.jpg")
...
#  используем группы что-бы добавить наши функции
...
while True:
    ...
    if pygame.sprite.spritecollideany(P1, allcoins):
        COIN += 1
        pygame.mixer.music.load('racer/coins_sound.mp3')
        pygame.mixer.music.play(start=0.6)
        C1.coincollect()

    # будет запущен, если произойдет столкновение между игроком и врагом
    if pygame.sprite.spritecollideany(P1, enemies):
        pygame.mixer.Sound('racer/car_crash.wav').play()
        time.sleep(0.5)

        DISPLAYSURF.fill(RED)
        DISPLAYSURF.blit(game_over, (120, 250))
